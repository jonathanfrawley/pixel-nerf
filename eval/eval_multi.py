"""
Eval on real images from input-multi/*_normalize.png, output to output/

* should be the angle the image is rotated to i.e. front should be 180_normalize.png and side view should be 90_normalize.png
...
def extra_args(parser):
    parser.add_argument(
        ...
        "-I",
        type=str,
        default=os.path.join(ROOT_DIR, "input-multi"),
...
cam_angles = angles = [int(x.split('_')[0]) for x in inputs_all if x.endswith("_normalize.png")]
cam_poses = torch.stack(
    [   _coord_from_blender @ util.pose_spherical(angle, args.elevation, args.radius)
        for angle in cam_angles
    ],
    0
).to(device)
print(f"Found {len(cam_poses)} images at angles {cam_angles}")
